...
- **Save new contacts** (great with the business-card scan below)
...
## How it's built (6 workflows)

The main workflow calls five small **sub-workflows** ("helpers") as tools. They exist to work
around a few n8n quirks: the Calendar `getAll` node ignores date windows, some create-time options
get dropped by the tool node, and there's no create-contact tool node — so those go through direct
API calls.
...
the CLI to import the six workflows, re-link them, and guide you through credentials.
...
In n8n: **Workflows → Import from File**, and import all six. **Do the five helpers
...
| **Google OAuth2 API** *(for contacts)* | Saved-contacts search + creating contacts — see step 6 |
...
- **Helpers**: Calendar helpers → Google Calendar; Contacts Search → Gmail (Search Gmail) +
  Google OAuth2 (List Contacts); Contact Create → Google OAuth2 (Create Contact).
...
the helper in each of these five tool nodes (click the node → Workflow dropdown → pick the
...
### 6. Google Contacts (search + create) via the People API
Both the contacts **search** (`List Contacts`) and **create** (`Create Contact`) nodes use a
**Google OAuth2 API** credential. n8n's one-click Google sign-in can't grant the contacts scope,
so create your own Google Cloud OAuth client:
...
2. Configure the **OAuth consent screen**; add the scope. Search-only works with
   `…/auth/contacts.readonly`, but to also **create** contacts (the business-card feature) use the
   read-write scope **`https://www.googleapis.com/auth/contacts`** — it covers both, so just use it.
...
4. In n8n, create a **Google OAuth2 API** credential with that client ID/secret + the scope,
   **connect it (complete the Google sign-in)**, and turn on **"Allow use in HTTP Request node."**
   Attach it to `List Contacts` and `Create Contact`.
...
> Otherwise contact creation fails with **403 Forbidden** even though the scope looks correct.

If you skip the People API, contact *search* still falls back to Gmail-only (that node is set to
continue-on-error), but saving contacts and saved-contact lookups won't work.
